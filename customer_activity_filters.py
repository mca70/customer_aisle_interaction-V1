...
class HumanTracker:
    ...
    def process_frame(self, frame):
        ...
        for box, track_id, keypoint in zip(boxes, track_ids, keypoints):
            stationary_detected = self._process_track(box, track_id, keypoint, annotated_frame)
            if stationary_detected:
                self.stationary_detected = True  # Set the flag to True
                return annotated_frame  # Early exit once a stationary customer is detected

        return annotated_frame

    def _process_track(self, box, track_id, keypoint, frame):
        x, y, w, h = box
    ...
    def run(self, video_path):
        ...
        while self.cap.isOpened():
            ...
            if success:
                check_staff = detect_staff_and_trolley(self.staff_model, frame)
                if check_staff:
                    staff_bbox_count += 1
                ...
                check_trolley = detect_staff_and_trolley(self.trolley_model, frame)
                if check_trolley:
                    trolley_bbox_count += 1
                ...
                if self.frame_count > self.fps * 18:
                    return False
            else:
                break
...
try:
    ...
    start = time.time()
    out = track.run('../data/2024-09-09_ietr/videos/118/15.mp4')
